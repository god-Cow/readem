...
Page({
  ...
  onLoad: function (options) {
    ...
    that.setData({
      pageCome:path.split("_")[0],
      cartId: path.split("_")[2],
      skuId: path.split("_")[1],
      money: {}
    })
    if (that.data.pageCome == 2 || that.data.pageCome == 3){
      that.setData({
        groupId: path.split("_")[3]
      })
    }
    if (that.data.pageCome == 4){
      that.setData({
        activityType: path.split("_")[3]
      })
    }
    wx.removeStorage({
      key: 'discount',
        ...
        wx.request({
          ...
          success: function (result) {
            ...
            if (result.data.status == 2000) {
              ...
              var flag = result.data.data.flag;
                        ...
                        wx.requestPayment(
                          {
                            ...
                            'success': function (res) {
                              ...
                              if (that.data.pageCome == 1){
                                wx.navigateTo({
                                  url: '../success/success?orderId=' + orderSnGo,
                                })
                              }
                              if(that.data.pageCome==2){
                                wx.navigateTo({
                                  url: '../successGroupOrder/successGroupOrder?path=' + that.data.groupId + "_" + orderSnGo,
                                })
                              }
                              if (that.data.pageCome == 3) {
                                wx.navigateTo({
                                  url: '../doubleSuccessGroup/doubleSuccessGroup?path=' + that.data.groupId + "_" + orderSnGo,
                                })
                              }
                              if (that.data.pageCome == 4) {
                                if (flag == 1) {
                                  wx.navigateTo({
                                    url: '../collagePaysucess/collagePaysucess?path=' + orderSnGo,
                                  })
                                } else {
                                  wx.navigateTo({
                            ...
                            'fail': function (res) {
                              if (that.data.pageCome == 4) {
                                if (flag == 1){
                                  wx.navigateTo({
                                    url: '../collageDetail/collageDetail?orderId=' + orderSnGo,
                                  })
                                }else{
                                  wx.navigateTo({
                                    url: '../orderInfo/orderInfo?orderId=' + orderSnGo,
                                  })
                                }
                              } else {
                                wx.navigateTo({
  ...
  discount: function () {
    ...
    //               setTimeout(function () {
    //                 wx.navigateTo({
    //                   url: '../selectDiscount/selectDiscount?cartId=' + that.data.cartId + "&pageCome=2&skuId=" + that.data.skuId,
    //                 })
    //               }, 2000)
    //             } else {
    //               that.setData({
  ...
  giftCard: function () {
    wx.navigateTo({
      url: '../selectGiftCard/selectGiftCard?cartId=' + this.data.cartId + "&pageCome=2&skuId=" + this.data.skuId,
    })
  },
  getAllMoney: function (expressId, point, giftCardNumber, discountCardNumber){
    ...
    wx.request({
      url: app.api.getSingleSettlement,
      data: {
        token: that.data.token,
        phonetype: that.data.phoneType,
        channel: 6,
