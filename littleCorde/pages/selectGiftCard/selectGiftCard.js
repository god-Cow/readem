  ...
  disCountClick: function (e) {
    ...
    that.data.coupon.forEach(function (v, i) {
      if (v.cardNumber == index) {
        if (v.flag) {
          v.flag = false;
          for (var a = 0; a < that.data.selectGiftCard.length; a++) {
            if (that.data.selectGiftCard[a] == v.cardNumber) {
              that.data.selectGiftCard.splice(a, 1)
            }
          }
        } else {
          v.flag = true;
          that.data.selectGiftCard.push(v.cardNumber)
        }
      }
    })
    that.setData({
      coupon: that.data.coupon,
      selectGiftCard: that.data.selectGiftCard
    })
  },
  convert: function () {
    var that =this;
          ...
          if (newRes.data.status == 2000) {
            ...
            for (var i = 0; i < newRes.data.data.length; i++) {
              that.data.coupon.push({
                ...
                surplus: newRes.data.data[i].surplus,
            ...
            wx.getStorage({
              ...
              success: function (res) {
                that.setData({
                  selectGiftCard: res.data
                })
                var discount = that.data.selectGiftCard;
                that.data.coupon.forEach(function (v, i) {
                  for (var a = 0; a < discount.length; a++) {
                    if (v.cardNumber == discount[a]) {
                      v.flag = true;
                    }
                  }
                })
